...
static unsigned char reg_f4_value = 0x33;	//0x2b;		// bit[5:7] temp_average  bit[2:4] press_average  bit[0:1] power_mode
                         ...
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    ...
    if(cali_read_reg == 0)
    {
    	memset(cali_reg, 0 ,sizeof(cali_reg));
        mTask.txBuf[0] = QMP6988_CALIBRATION_DATA_START;
        ret += i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, (uint8_t*)&mTask.txBuf[0], 1,(uint8_t*)&cali_reg[0], 6, i2cCallBack,(void *)STATE_IDLE);

        mTask.txBuf[1] = QMP6988_CALIBRATION_DATA_START + 6;
        ret += i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, (uint8_t*)&mTask.txBuf[1], 1,(uint8_t*)&cali_reg[6], 6, i2cCallBack,(void *)STATE_IDLE);		

        mTask.txBuf[2] = QMP6988_CALIBRATION_DATA_START + 12;
        ret += i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, (uint8_t*)&mTask.txBuf[2], 1,(uint8_t*)&cali_reg[12], 6, i2cCallBack,(void *)STATE_IDLE);

        mTask.txBuf[3] = QMP6988_CALIBRATION_DATA_START+ 18;
        ret += i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, (uint8_t*)&mTask.txBuf[3], 1,(uint8_t*)&cali_reg[18], 6, i2cCallBack,(void *)STATE_IDLE);

        mTask.txBuf[4] = QMP6988_CALIBRATION_DATA_START+ 24;
        ret += i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, (uint8_t*)&mTask.txBuf[4], 1,(uint8_t*)&cali_reg[24], 1, i2cCallBack,(void *)STATE_IDLE);
    ...
    mTask.txBuf[0] = 0xF7;
    ret = i2cMasterTxRx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 1,mTask.rxBuf, 6, i2cCallBack,next_state);
...
void qmp6988_get_cali_reg(void)
{
	int ret=0;
	int i=0;
	...
	{
	    mTask.txBuf[0] = QMP6988_CALIBRATION_DATA_START+i;
        ret = i2cMasterTxRxSync(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 1,&cali_reg[i], 1, NULL, NULL);
		//osLog(LOG_ERROR,"cali_reg[%d] = %d ", i, cali_reg[i]);
		if(ret < 0)
		{
			osLog(LOG_ERROR,"qmp6988_get_cali_reg error\n");
			break;
		}
	}

	if(i >= QMP6988_CALIBRATION_DATA_LENGTH)
		cali_read_reg = 1;
}
                         ...
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
	...
	mTask.txBuf[0] = QMP6988_CTRLMEAS_REG;
	mTask.txBuf[1] = (reg_f4_value&0xfc)|QMP6988_SLEEP_MODE;		//reg_f4_value;
	...
	mTask.txBuf[1] = 0x02;			// IIR filter 4
    ret = i2cMasterTx(mTask.hw->i2c_num, mTask.i2c_addr, mTask.txBuf, 2, i2cCallBack, (void *)STATE_IDLE);
	mTask.txBuf[0] = QMP6988_IO_SETUP_REG;
	mTask.txBuf[1] = 0x00;			//bit[5:7]	standby time
...
static int qmp6988_compParams2(I2cCallbackF i2cCallBack, SpiCbkF spiCallBack, void *next_state,
                         void *inBuf, uint8_t inSize, uint8_t elemInSize,
                         void *outBuf, uint8_t *outSize, uint8_t *elemOutSize)
{
    int ret = 0;
...
static struct sensorFsm qmp6988Fsm[] = {
    ...
    sensorFsmCmd(STATE_CONFIG, STATE_CORE, qmp6988_compParams2),
...
static int qmp6988Init(void)
{
	...
	if(ret < 0)
	{
	    mTask.i2c_addr = mTask.hw->i2c_addr[1];
	    osLog(LOG_ERROR, "i2c_num: %d, i2c_addr: 0x%x\n\r", mTask.hw->i2c_num, mTask.i2c_addr);
	    mTask.txBuf[0] = QMP6988_CHIP_ID_REG;
