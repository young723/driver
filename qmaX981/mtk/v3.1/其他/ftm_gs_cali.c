 ...
 * Functions 
 *****************************************************************************/
#define ACC_QST_CALI_SUPPORT
#if defined(ACC_QST_CALI_SUPPORT)
#define ACC_CALI_DATA_NUM		20

int gsensor_qst_cali(int fd, int side)
{
	int err, icount;
	int x, y, z;
	char xyz_buf[128];
	int acc_x_sum,acc_y_sum,acc_z_sum;
	HwmData cali;
	
	if(fd < 0)
	{
		GSCLOGD("gsensor_qst_cali fd error\n");
		return -1;
	}
	
	err = ioctl(fd, GSENSOR_IOCTL_CLR_CALI, NULL);
	if(err)
	{
		GSCLOGE("gsensor_qst_cali GSENSOR_IOCTL_CLR_CALI error\n");
		return -1;
	}
	// clear nvram
	...
	{	
		err = ioctl(fd, GSENSOR_IOCTL_READ_SENSORDATA, xyz_buf);
		usleep(5*1000);
	}
	acc_x_sum = 0;
	acc_y_sum = 0;
	acc_z_sum = 0;
	for(icount=0; icount<ACC_CALI_DATA_NUM; icount++)
	{
		err = ioctl(fd, GSENSOR_IOCTL_READ_SENSORDATA, xyz_buf);
		if(err)
		{
			GSCLOGE("gsensor_qst_cali read data fail: (%d)\n", err);
			return -1;
		}
		else if(3 != sscanf(xyz_buf, "%x %x %x", &x, &y, &z))
		{
			GSCLOGE("gsensor_qst_cali read format fail: (%d)\n", err);
		}
		else
		{
			acc_x_sum += x;
			acc_y_sum += y;
			acc_z_sum += z;
			GSCLOGD("gsensor_qst_cali acc data:(%d %d %d)\n", x,y,z);
		}		
		usleep(10*1000);
	}
	cali.rx = 0-(acc_x_sum/ACC_CALI_DATA_NUM);
	cali.ry = 0-(acc_y_sum/ACC_CALI_DATA_NUM);
	if(side < 0)
		cali.rz = -9807-(acc_z_sum/ACC_CALI_DATA_NUM);
	else
		cali.rz = 9807-(acc_z_sum/ACC_CALI_DATA_NUM);
	GSCLOGD("calc cali data:(%d %d %d)\n", cali.rx,cali.ry,cali.rz);
	//err = gsensor_set_cali(fd, &cali);
	err = ioctl(fd, GSENSOR_IOCTL_SET_CALI, &cali);
	if(err)
	{
		GSCLOGE("set calibration fail: %d\n", err);		
		return -1;
	}
	err = gsensor_get_cali(fd, &cali);		
	if(err)
	{	
		GSCLOGE("get calibration fail: %d\n", err);		
		return -1;
	}
	GSCLOGD("gsensor_get_cali:(%d %d %d)\n", cali.rx,cali.ry,cali.rz);
	err = gsensor_write_nvram(&cali);
	if(err)
	{
		GSCLOGE("write nvram fail: %d\n", err);
		return -1;
	}
...
static void *gs_cali_update_iv_thread(void *priv)
{
	...
	while(1)
	{
		...
		if(op == GS_OP_CLEAR)
		{
			...
			if(err)
			{
				len = snprintf(dat->gsc.status, sizeof(dat->gsc.status), "%s\n", uistr_info_sensor_cali_fail);
    ...
    flags = false;
    ioctl(gsc->fd, GSENSOR_IOCTL_INIT, &flags);
	gs_cali_close(gsc);
	LOGD(TAG "%s: Exit\n", __FUNCTION__);
	pthread_exit(NULL);
