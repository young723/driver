...
#define ACC_ATA_AUTO_CALI_SUPPORT
#define ACC_ATA_CHECK_STICK

#if defined(ACC_ATA_CHECK_STICK)
...
static int gsensor_stick_flag = 0;
#endif

#if defined(ACC_ATA_AUTO_CALI_SUPPORT)
extern int gsensor_qst_cali(int fd, int side);
#define ACC_ATA_AUTO_CALI_SIDE		1
#endif
...
static int gsensor_open(struct acc_priv *acc)
{
    ...
    if (err) {
        FTGLOGE("enable gsensor fail: %s", strerror(errno));
        return -1;            
    }
	return 0;	
}
...
static int gsensor_close(struct acc_priv *acc)
{
    ...
    if(acc->fd != 0)
    {
        ioctl(acc->fd, GSENSOR_IOCTL_INIT, &flags);
		close(acc->fd);
        acc->fd = -1;
	}
	acc->pos_chk = 0x00;
...
// add by yangzhiqiang
#if defined(ACC_ATA_CHECK_STICK)
static void gsensor_stick_check_set_reg(int fd, int flag)
{
	int err;
	
	err = ioctl(fd, GSENSOR_IOCTL_SET_STICK_CHECK_REG, flag);
	if(err)
		FTGLOGD("GSENSOR_IOCTL_SET_STICK_CHECK_REG err=%d", err);
}

static void gsensor_get_stick_status(int fd)
{
	int err;
	int stick_status;
	static char buf[64];
	...
	err = ioctl(fd, GSENSOR_IOCTL_GET_STICK_STATUS, buf);
	if(err == 0)
	{
		if(1 != sscanf(buf, "%d", &stick_status))
		{		
			FTGLOGD("sscanf data err=%d");
			gsensor_stick_flag = 0;
			return;
		}
		gsensor_stick_flag = stick_status;
		FTGLOGD("gsensor_stick_flag=%d", gsensor_stick_flag);
	}
	else
	{
		gsensor_stick_flag = 0;
...
// add by yangzhiqiang
	...
	{	
		FTGLOGD("ata mode, check stick start!");
		gsensor_stick_check_set_reg(acc->fd, 1);
		usleep(5*1000);
		gsensor_get_stick_status(acc->fd);
		gsensor_stick_check_set_reg(acc->fd, 0);
		usleep(5*1000);
	}
#endif
#if defined(ACC_ATA_AUTO_CALI_SUPPORT)
	if(sp_ata_status == FTM_AUTO_ITEM)
	{
		FTGLOGD("ata mode, auto cali, use gsensor_qst_cali!");
	#if defined(ACC_ATA_CHECK_STICK)
		if(gsensor_stick_flag == 0)
		{
			gsensor_qst_cali(acc->fd, ACC_ATA_AUTO_CALI_SIDE);
		}
	#else		
		gsensor_qst_cali(acc->fd, ACC_ATA_AUTO_CALI_SIDE);
	#endif
