...
#define HALL_NAME	"mtk-hall"  //hct-drv add for hall by qhs start
...
//add for intercom key report by zhuguanglei begin 20171127 
/*
	pressed: 	1==> 按键按下	|| 0==>按键抬起
	key_value: 	键值,在实际调用的地方传递进来
 */
#ifdef __HCT_INTERCOM_SUPPORT__
#if __HCT_INTERCOM_SUPPORT__
void intercomkey_handler(bool pressed, int key_value)
{
	printk("intercomkey_handler report key. \n");
	if(pressed)
	{
		printk("intercom key pressed. \n");
		input_report_key(kpd_input_dev, key_value, 1);
	}
	else
	{
		printk("intercom key released. \n");
...
#endif
#endif
//add for intercom key report by zhuguanglei end 20171127 
...
void kpd_send_powerkey(int mode)
{
	if(((mt_get_bl_brightness() == 0)&&(mode == 0)) || ((mt_get_bl_brightness() > 0)&&(mode == 1)))
	{
		if(test_bit(KEY_POWER, kpd_input_dev->keybit))
		{
			printk("[kpd_send_powerkey] PRESS POWER KEY!!\n");
			input_report_key(kpd_input_dev, KEY_POWER, 1);
			input_sync(kpd_input_dev);
			printk("[kpd_send_powerkey] RELEASE POWER KEY!!\n");
			input_report_key(kpd_input_dev, KEY_POWER, 0);
			input_sync(kpd_input_dev);
		}
		else
		{
			printk("[kpd_send_powerkey] Not Support POWER KEY!!\n");
		}
	}
	else
	{
		printk("[kpd_send_powerkey] backlight is on!!\n");
	}
}
// yangzhiqiang
...
/* hct-drv add fot hall by qhs start */
#if  __HCT_HALL_SUPPORT__
...
#endif
/* hct-drv add fot hall by qhs end */
...
#ifdef __HCT_INTERCOM_SUPPORT__
#if __HCT_INTERCOM_SUPPORT__
    kpd_print("__set_bit F9 begin.\n");
	__set_bit(KEY_F9, kpd_input_dev->keybit);
    kpd_print("__set_bit F9 end.\n");
