...
// add by yangzhiqiang for report data when control stepcounter 2016/11/28	
#if defined(SC_QST_MODIFY)
	step_c_report_rel();
#endif
// yangzhiqiang
...
static ssize_t step_c_store_flush(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct step_c_context *devobj = NULL;
	mutex_lock(&step_c_context_obj->step_c_op_mutex);
	//struct step_c_context *devobj = (struct step_c_context *)dev_get_drvdata(dev);
	devobj = (struct step_c_context *)dev_get_drvdata(dev);
...
static int step_c_misc_init(struct step_c_context *cxt)
{
	...
	cxt->mdev.minor = MISC_DYNAMIC_MINOR;//M_STEP_C_MISC_MINOR;
...
// add by yangzhiqiang for report x by rel, fix the bug ,apk do not refresh
#if defined(SC_QST_MODIFY)
...
{
	//STEP_C_LOG("+step_c_data_report! %d, %d\n",value,status);
	input_report_abs(dev, EVENT_TYPE_STEP_C_VALUE, value);
	input_report_abs(dev, EVENT_TYPE_STEP_C_STATUS, status);
	input_sync(dev);
	return 0;
}
// add by yangzhiqiang for report data by REL
#if defined(SC_QST_MODIFY)
void step_c_set_delay(int new_mdelay)
{
	if(step_c_context_obj)
	{
		atomic_set(&step_c_context_obj->delay, new_mdelay);
	}
}
...
if (NULL == step_c_context_obj->step_c_data.get_data)
	return;

	err = step_c_context_obj->step_c_data.get_data(&value,&status);
	
	if(err)
	{
		STEP_C_ERR("step_c_report_rel get step_c data fails!!\n" );
	}
	else
	{
		time.tv_sec = time.tv_nsec = 0;    
		time = get_monotonic_coarse(); 
		nt = time.tv_sec*1000000000LL+time.tv_nsec;
		step_c_context_obj->drv_data.step_c_data.values[0] = value;
		step_c_context_obj->drv_data.step_c_data.status = status;
		step_c_context_obj->drv_data.step_c_data.time = nt;

		STEP_C_LOG("yzq rel data=%d \n", step_c_context_obj->drv_data.step_c_data.values[0]);
		input_report_rel(step_c_context_obj->idev, REL_X, step_c_context_obj->drv_data.step_c_data.values[0]);
		input_sync(step_c_context_obj->idev);
	}
}
#endif
// yangzhiqiang
