	...
	struct qma6981_data *sensor)
{
	...
	input_set_capability(sensor->stepcount_dev, EV_ABS, ABS_MISC);
...
#if defined(QMA6981_STEPCOUNTER)	
	data[QMA6981_AXIS_X] -= 100;
	data[QMA6981_AXIS_Y] -= 100;
	data[QMA6981_AXIS_Z] -= 100;
#endif	
...
int qma6981_sc_get_data(void)
{
	...
	if(diff > QMA6981_ABNORMAL_CHECK)
	{
		...
		if((QMA6981_ABS(g_qma6981_data_c.more_data[0]-g_qma6981_data_c.curr_data) > 2)
			||(QMA6981_ABS(g_qma6981_data_c.more_data[0]-g_qma6981_data_c.curr_data) > 2)
			||(QMA6981_ABS(g_qma6981_data_c.more_data[0]-g_qma6981_data_c.curr_data) > 2)
			)
		{
			qma6981_set_wakelock(0);
	...
	if(gpio_is_valid(acc_qst->int_pin))
	{
		...
		if (acc_qst->irq){
				err = request_threaded_irq(
						...
						NULL,
						qma6981_irq,
						IRQF_TRIGGER_HIGH,	//IRQF_TRIGGER_HIGH|IRQF_ONESHOT,
