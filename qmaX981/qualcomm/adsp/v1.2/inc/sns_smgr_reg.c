...
/* $Header: //components/rel/ssc.adsp/2.6.1/smgr/src/sns_smgr_reg.c#2 $ */
...
#endif
#ifdef CONFIG_SUPPORT_QMAX981
	{ SNS_REG_UUID_QMAX981, & sns_dd_qmaX981_if},
#endif
  ...
  else
  {
                  ...
                  ix < SNS_REG_SSI_SMGR_CFG_NUM_SENSORS; ix++ )
    {
      ...
      SNS_OS_MEMCOPY(sensor_cfg_ptr->uuid, cfg_group_ptr->drv_cfg[ix].drvuuid, 
                     sizeof(sensor_cfg_ptr->uuid));
      sensor_cfg_ptr->off_to_idle_time = (uint16_t)sns_em_convert_usec_to_dspstick(cfg_group_ptr->drv_cfg[ix].off_to_idle);
      sensor_cfg_ptr->idle_to_ready_time = (uint16_t)sns_em_convert_usec_to_dspstick(cfg_group_ptr->drv_cfg[ix].idle_to_ready);
