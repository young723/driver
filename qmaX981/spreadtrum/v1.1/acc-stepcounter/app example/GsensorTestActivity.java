...
public class GsensorTestActivity extends BaseActivity {
    ...
    private boolean mIsShowDialog = true;
    	// add by yangzhiqiang		
    File file_cali = new File("/sys/class/input/input1/cali");
	// add by yangzhiqiang
    ...
    private static String writeFile(File fn) {
        FileWriter f;
        int len;
    ...
    public void onClick(View v) {
      ...
      if (v == mPassButton) {
      	super.onClick(v);
					...
					if("Cali".equals(mFailButton.getText().toString())){
	 					Log.d("gsensor cali", "Cali button press");
	 					writeFile(file_cali);
	 					mFailButton.setText("Fail");						
					}else{
						super.onClick(v);
					}
        }
