...
static struct sensor_t sSensorList[AccSensor::numSensors
				   ...
				   PlsSensor::numSensors+StepCounterSensor::numSensors] = {
};
...
sensors_poll_context_t::sensors_poll_context_t()
{
	...
	numSensors +=
		mSensors[stepcounter]->populateSensorList(sSensorList + numSensors);
    mPollFds[stepcounter].fd = mSensors[stepcounter]->getFd();
    mPollFds[stepcounter].events = POLLIN;
    mPollFds[stepcounter].revents = 0;
	ALOGD("SCnumSensors=%d; %d", numSensors, StepCounterSensor::numSensors);
#endif 
#ifndef ORI_NULL
...
sensors_poll_context_t::~sensors_poll_context_t()
{
	...
	ALOGD("sensor_hal ~sensors_poll_context_t call\n");
...
int sensors_poll_context_t::activate(int handle, int enabled)
{
	...
	ALOGD("qst_activate handle=%d; drv=%d", handle, drv);
