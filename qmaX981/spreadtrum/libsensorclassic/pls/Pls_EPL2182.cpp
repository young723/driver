...
PlsSensor::PlsSensor()
    : SensorBase(ELAN_DEVICE_NAME, "alps_pxy"),
      mEnabled(0),
      mPendingMask(0),
      mInputReader(32),
...
bool PlsSensor::hasPendingEvents() const { return mHasPendingEvent; }

int PlsSensor::setDelay(int32_t handle, int64_t ns) { return 0; }
...
int PlsSensor::setEnable(int32_t handle, int en) {
  ...
  if (uint32_t(what) >= numSensors) return -EINVAL;

  int newState = en ? 1 : 0;
  int err = 0;

  ALOGD("PlsSensor::enable en=%d; newState=%d; what=%d; mEnabled=%d\n", en,
        newState, what, mEnabled);
...
int PlsSensor::readEvents(sensors_event_t *data, int count) {
  ...
  while (count && mInputReader.readEvent(&event)) {
    ...
    if (type == EV_ABS) {
      ...
      ALOGE("PlsSensor: unknown event (type=%d, code=%d)", type, event->code);
      mInputReader.next();
    }
  }
...
void PlsSensor::processEvent(int code, int value) {
  switch (code) {
    ...
    case EVENT_TYPE_LIGHT:
      ...
      mPendingEvents[Light].light = static_cast<float>(value);
