...
#ifndef ORI_NULL
#include "mag/OriSensor.h"
#endif
#ifndef STEP_C_NULL
#include "step_c/StepCounter.h"
#endif
...
sensors_poll_context_t::sensors_poll_context_t() {
  ...
  ALOGD("numSensors=%d, PlsSensor::numSensors%d", numSensors, PlsSensor::numSensors);
	...
	numSensors += mSensors[step_c]->populateSensorList(sSensorList + numSensors);
	mPollFds[step_c].fd = mSensors[step_c]->getFd();
	mPollFds[step_c].events = POLLIN;
	mPollFds[step_c].revents = 0;
	ALOGD("numSensors=%d, StepCounterSensor::numSensors%d", numSensors, StepCounterSensor::numSensors);
#endif
