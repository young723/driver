...
#define Android_Marshmallow				//Android 6.0 7.0
...
#define QST_VIRTUAL_SENSOR  //defined using new arch for virtual sensor
//#define QST_Dummygyro_VirtualSensors	//defined using old arch for virtual sensors
//#define QST_Dummygyro		// defineD using old arch for PG
...
#define G2UT 100  
#define G2MG 1000
static int g_magData_unit = G2MG;
...
static int qmcX983_read_mag_xyz(int *data)
{
	...
	while(!(rdy & 0x07) && (t1 < 3)){
		...
		rdy=databuf[0];
		MSE_LOG("QMCX983 Status register is (%2x)\n", rdy);
		t1 ++;
	}
	...
	hw_d[0] = hw_d[0] * g_magData_unit / clientdata->xy_sensitivity;
	hw_d[1] = hw_d[1] * g_magData_unit / clientdata->xy_sensitivity;
	hw_d[2] = hw_d[2] * g_magData_unit / clientdata->z_sensitivity;
	...
	if(upmu_is_chr_det()==KAL_TRUE)
	{
		...
		else
		{
			...
			hw_d[0]=hw_d[0]-COMPENSATE[0]*ICharging_mag/1000;
			hw_d[1]=hw_d[1]-COMPENSATE[1]*ICharging_mag/1000;
			hw_d[2]=hw_d[2]-COMPENSATE[2]*ICharging_mag/1000;
...
static ssize_t store_registers_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	sscanf(buf,"0x%2x",(unsigned char*)&tempbuf[0]);
	
	data[1] = tempbuf[0];
	data[0] = regbuf[0];
	err = I2C_TxData(data, 2);
	if(err != 0)
	   MSE_ERR("%s Reg[0x%2x] ---> 0x%2x\n",__func__,regbuf[0],tempbuf[0]);

	return count;
}

static ssize_t show_registers_addr(struct device_driver *ddri, char *buf)
{
...
static ssize_t store_registers_addr(struct device_driver *ddri, const char *buf, size_t count)
{
	struct qmcX983_i2c_data *obj = i2c_get_clientdata(this_client);
	...
	if(1 == sscanf(buf,"0x%2x",(unsigned char*)&regbuf[0])){
...
static DRIVER_ATTR(registers_value, S_IRUGO | S_IWUSR, show_registers_value, store_registers_value);
static DRIVER_ATTR(registers_addr, S_IRUGO | S_IWUSR, show_registers_addr, store_registers_addr);
...
static struct driver_attribute *qmcX983_attr_list[] = {
    ...
    &driver_attr_registers_value,
	&driver_attr_registers_addr,
    &driver_attr_daemon,
...
static long qmcX983_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case QMC_IOCTL_READ_SENSORDATA:   // for daemon
		if(argp == NULL){
...
/*------------------------------for daemon------------------------*/
	...
	case MSENSOR_IOCTL_READ_SENSORDATA:	//for ATA
		if(argp == NULL)
		{
...
static long qmcX983_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_QMC_IOCTL_SET_RANGE:
	...
	/* ================================================================ */
	case COMPAT_QMC_IOCTL_SET_MODE:
	...
	/* ================================================================ */
	case COMPAT_QMC_IOCTL_READ_SENSORDATA:
...
static int qmcX983_gs_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*x = sensor_data[12] * CONVERT_Q16;
	*y = sensor_data[13] * CONVERT_Q16;
	*z = sensor_data[14] * CONVERT_Q16;
	*status = sensor_data[7];
...
static int qmcX983_rv_get_data(int* w, int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
	
	*x = sensor_data[16] * CONVERT_Q16;
	*y = sensor_data[17] * CONVERT_Q16;
	*z = sensor_data[18] * CONVERT_Q16;
	*w = sensor_data[19] * CONVERT_Q16;
	*status = sensor_data[7];
...
static int qmcX983_gr_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*x = sensor_data[20] * CONVERT_Q16;
	*y = sensor_data[21] * CONVERT_Q16;
	*z = sensor_data[22] * CONVERT_Q16;
	*status = sensor_data[7];	
...
static int qmcX983_la_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*x = sensor_data[24] * CONVERT_Q16;
	*y = sensor_data[25] * CONVERT_Q16;
	*z = sensor_data[26] * CONVERT_Q16;
	*status = sensor_data[7];
...
#ifdef QST_VIRTUAL_SENSOR
	mag_data.div_gs = CONVERT_Q16_DIV;
	mag_data.div_rv = CONVERT_Q16_DIV;
	mag_data.div_gr = CONVERT_Q16_DIV;
	mag_data.div_la = CONVERT_Q16_DIV;
#endif
...
#else
	qst_hw = qmcX983_get_cust_mag_hw();
